...
class bm_analyze(object):
    ...
    def bm_search(self):
        ...
        for sub_len in range(self.min_len,self.max_len+1):
            ...
            #i is p
                ...
                if self.sub_dict.get(hash(tuple(sub_seq)))==None:
                    ...
                    self.sub_dict[hash(tuple(sub_seq))]=len(pattern_array)
        ...
        color_value=[int(v*20) for v in color_value]
        print (color_value)
        ...
        for i in range(len(self.sub_seq_information)):
            ...
            for p in position:
                for j in range(temp_len):
                    self.color_value[p+j]=max(value,self.color_value[p+j])
...
if __name__=='__main__':
    ...
    c=bm_analyze(20,100,[int(x) for x in b.randomlize_xlist])
    c.bm_search()
    ...
    draw_pic.draw_pic([int(x) for x in b.randomlize_xlist],c.color_value,a)
    pass